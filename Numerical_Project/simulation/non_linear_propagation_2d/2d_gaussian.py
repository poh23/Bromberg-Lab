...
def gaussian_2d_with_lens():
    ...
    kerr_coefficient = -1e-11 # when kerr_coefficient is larger than 1e-3 there isn't energy conservation
    # Linear Propagation
    split_step = SplitStep2d(kerr_coefficient=0)
    f = -5e-3 # focal length of the lens in meters
    init_envelope = A * np.exp(-(X ** 2 + Y ** 2) / sigma ** 2) * np.exp(-1j * split_step.k * (X ** 2 + Y ** 2) / (2*f) )  # Gaussian beam with a phase front
    z, total_envelope_linear, total_energies_linear = split_step.propagate(L, x, x, init_envelope)
    # take y=0 slice of the total envelope
    y_slice = 0
    x = X[0, :]
    y = closest(Y[:, 0], y_slice)

    Intensity_linear = np.abs(total_envelope_linear) ** 2
    # take y=0 slice of the intensity
    Intensity_linear_slice = Intensity_linear[:, y]
    axes[0] = XY_2d_heatmap(axes[0], fig, X, Y, Intensity_linear[0], 'Initial Intensity', cmap='viridis')
    axes[1] = XY_2d_heatmap(axes[1], fig, X, Y, Intensity_linear[-1], 'Final Intensity after linear propagation',
                            cmap='viridis')
    ...
    fig1, ax = plt.subplots(1, 1, figsize=(5, 15))
    X, Z = np.meshgrid(x, z)
    XY_2d_heatmap(ax, fig1, X, Z, Intensity_linear_slice, 'Final Intensity after linear propagation',
                            cmap='viridis')
    plt.show()
