...
class Speckles:
    ...
    # defing k-space
    fs = 1 / (XX.max() - XX.min())  # sapatial freq = 1/x
    ...
    def _get_prop_mat(self,l,rev=False):
        if rev == True:
            return np.exp(-1j * SanityClass.k_z * l)
        return np.exp(+1j * SanityClass.k_z * l)
    ...
    def phase_mask_macro_pixels_method(self,XX, macro_pixel_size)->np.ndarray:
        init_mask = np.random.uniform(0, 2*np.pi, size=(SanityClass.Nx//macro_pixel_size, SanityClass.Ny//macro_pixel_size))
        # resize: start with 256X256 random, and then resize to 1024X1024 for macro pixels 
        phase_mask = cv2.resize(init_mask, XX.shape[::-1], interpolation=cv2.INTER_AREA)
        return phase_mask
    ...
    def propagate_thick_with_angle(self,E_start,angle):
        E_init = E_start.copy()*np.exp(1j*angle*(SanityClass.XX+SanityClass.YY))
        ...
        E_diffuser2 = E* np.exp(1j*self.diffuser2_phase)
        E_out_fsp = self.free_space_propagation(E_diffuser2,self.L)
        E_clear = E_diffuser2/np.exp(1j*angle*(SanityClass.XX+SanityClass.YY))
    ...
    def calc_one_thick_diff_mem(self,d):
        ...
        E_start = np.exp(-(SanityClass.XX**2+SanityClass.YY**2)/(SanityClass.sig/30)**2) #the sigma of the focus i want to achieve - nack prop from here to find slm phase
        ...
        for i,phase in enumerate(phase_vec):
            a = (2*np.pi/SanityClass.wl)*np.sin(phase)
            E_start = np.exp(-(SanityClass.XX**2+SanityClass.YY**2)/(SanityClass.sig/5)**2) #the sigma of the laser, this one is important to speckle grain
...
# methods for mosk
    def get_cost(self,E_init):
        ...
        I = np.sum(np.abs(E_out[SanityClass.roi])**2)
        return I
    ...
    def find_best_phi(self,A, i, j,phase_vec,E_init):
        ...
        for ind, phase in enumerate(phase_vec):
            ...
            self.slm_phase[SanityClass.active_slice] = A_scaled
